Shader "AO/ScreenSpaceAOEffect"
{
	...
	{
		float linear01Depth , linear01Depth0 , linear01Depth1;
		float3 viewNormal;
		float3 viewNormal0;
		float3 viewNormal1;
		float2 delta = _MainTex_TexelSize.xy * 2;

		float4 cdn = tex2D(_CameraDepthNormalsTexture, i.uv);
		float4 cdn0 = tex2D(_CameraDepthNormalsTexture, i.uv - delta);
		float4 cdn1 = tex2D(_CameraDepthNormalsTexture, i.uv + delta);

		DecodeDepthNormal(cdn, linear01Depth, viewNormal);
		DecodeDepthNormal(cdn0, linear01Depth0, viewNormal0);
		DecodeDepthNormal(cdn1, linear01Depth1, viewNormal1);

		viewNormal = normalize(viewNormal) * float3(1, 1, -1);
		viewNormal0 = normalize(viewNormal0) * float3(1, 1, -1);
		viewNormal1 = normalize(viewNormal1) * float3(1, 1, -1);

		fixed4 col = tex2D(_MainTex, i.uv);

		float2 uv = i.uv;
		float2 uv0 = (i.uv[0], i.uv[1] - delta);
		float2 uv1 = (i.uv[0], i.uv[1] + delta);
		float3 normal = normalize(GetNormal(uv));
		float3 normal0 = normalize(GetNormal(uv0));
		float3 normal1 = normalize(GetNormal(uv1));
		float3 ViewR = normalize(i.viewRay) *float3(1, 1, -1);
		float dotv = dot(ViewR, viewNormal);
		float dot0 = dot(ViewR, viewNormal0);
		float dot1 = dot(ViewR, viewNormal1);

		dotv *= -1;
		dot0 *= -1;
		dot1 *= -1;

		if (dotv < dot0 - 0.1 && dotv < dot1 - 0.1)			
		{
			col.r = 0.0;
			col.g = 0.0;
			col.b = 0.0;
